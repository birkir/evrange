...
const distance = (a: GeoPoint, b: GeoPoint) => {
  const R = 6371;
...
export const calculateRoute = (speedster = 1.0) => {
  ...
  const result = route.routes[0].legs[0].steps.map(step => {
    const points = polyline.decode(step.polyline.points).reduce((acc: GeoPoint[], point: GeoPoint) => {
      const dist = distance(acc[acc.length - 1], point);
    ...
    return {
      ...
      points: points.map((from: GeoPoint, index: 0, arr: GeoPoint[]) => {
        ...
        const to = next ? next : [step.end_location.lat, step.end_location.lng, 'NO NEXT'];
        return [from, to];
      }),
    };
  });
  ...
  // elevation points
  const elevations = result.reduce((acc: string[], item: any) => {
  ...
  return result.map(step => {
    const speedInKmPerHour = step.distance.value / step.duration.value * 3600 / 1000 * speedster;
    ...
    // consumption base!
    let startElevation = elevation.results.find(el => el.location.lat === step.start_location.lat && el.location.lng === step.start_location.lng)!;
    let endElevation = elevation.results.find(el => el.location.lat === step.end_location.lat && el.location.lng === step.end_location.lng)!;
    let riseMeters = endElevation.elevation - startElevation.elevation;
    let grade = riseMeters / step.distance.value;
    let heightAboveReference = (startElevation.elevation + endElevation.elevation) / 2;
    ...
    const consumption = Consumption.create({
      ...
      cardinalDirection: cardinalDirection([step.start_location.lat, step.start_location.lng], [step.end_location.lat, step.end_location.lng]),
      windSpeed,
      windDirection,
    ...
    const consumptionSteps = step.points.map(([a, b]: [GeoPoint, GeoPoint], index: number, arr: any) => {

      startElevation = elevation.results.find(el => el.location.lat === a[0] && el.location.lng === a[1])!;
      endElevation = elevation.results.find(el => el.location.lat === b[0] && el.location.lng === b[1])!;
      ...
      const stepConsumption = Consumption.create({
        ...
        distance: arr.length === 1 ? step.distance.value : distance(a, b),
      } as any);
